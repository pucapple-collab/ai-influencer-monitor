...
type ProviderOps = { realExecutionEnabled:boolean; providers:Array<{id:string;name:string;configured:boolean;executionImplemented:boolean;executableNow:boolean}>; queue:{total:number;planned:number;blocked:number;completed:number;errors:number;byProvider:Record<string,number>}; audit:{total:number;externalCalls:number;errors:number;last:string|null}; collaboration:{reviews:number;improvements:number;errors:number;lastCrossReviewAt:string|null;realExternalCalls:number} };
type WidgetStatus = {
  priority: string;
  summary: {
    factoryRuns: number;
  ...
  return (
    <section className="mt-8 rounded-2xl border border-cyan-500/20 bg-cyan-500/[0.04] p-6">
      ...
      <div className="mt-5 rounded-xl border border-white/10 bg-black/20 p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-4 text-xs text-zinc-500"><span>Queue {providerOps?.queue.total ?? 0}</span><span>Planned {providerOps?.queue.planned ?? 0}</span><span>Blocked {providerOps?.queue.blocked ?? 0}</span><span>Done {providerOps?.queue.completed ?? 0}</span><span>Errors {providerOps?.queue.errors ?? 0}</span></div>\n        <div className="mt-4 grid gap-2 sm:grid-cols-5 text-xs"><div className="rounded-lg border border-white/10 p-3"><p className="text-zinc-600">REAL CALLS</p><p className="mt-1 text-lg">{providerOps?.collaboration.realExternalCalls ?? 0}</p></div><div className="rounded-lg border border-white/10 p-3"><p className="text-zinc-600">REVIEWS</p><p className="mt-1 text-lg">{providerOps?.collaboration.reviews ?? 0}</p></div><div className="rounded-lg border border-white/10 p-3"><p className="text-zinc-600">IMPROVEMENTS</p><p className="mt-1 text-lg">{providerOps?.collaboration.improvements ?? 0}</p></div><div className="rounded-lg border border-white/10 p-3"><p className="text-zinc-600">COLLAB ERRORS</p><p className="mt-1 text-lg">{providerOps?.collaboration.errors ?? 0}</p></div><div className="rounded-lg border border-white/10 p-3"><p className="text-zinc-600">LAST CROSS REVIEW</p><p className="mt-1 break-all">{providerOps?.collaboration.lastCrossReviewAt ?? "NOT PROVEN"}</p></div></div>
      </div>
