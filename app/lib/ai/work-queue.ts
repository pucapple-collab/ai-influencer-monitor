...
export type ProviderWorkItem={id:string;task:FactoryTaskKind;prompt:string;primary:string;fallback?:string;status:ProviderWorkStatus;executableNow:boolean;attempts?:number;lastProvider?:string;lastError?:string;externalCallMade?:boolean;createdAt:string;updatedAt:string};
...
export async function updateProviderWork(id:string,patch:Partial<Pick<ProviderWorkItem,"status"|"executableNow"|"attempts"|"lastProvider"|"lastError"|"externalCallMade">>){return locked(async()=>{const items=await readUnlocked();const index=items.findIndex(x=>x.id===id);if(index<0)return null;items[index]={...items[index],...patch,updatedAt:new Date().toISOString()};await writeUnlocked(items);return items[index];});}
