...
export async function updateProviderWork(id:string,patch:Partial<Pick<ProviderWorkItem,"status"|"executableNow">>){return locked(async()=>{const items=await readUnlocked();const index=items.findIndex(x=>x.id===id);if(index<0)return null;items[index]={...items[index],...patch,updatedAt:new Date().toISOString()};await writeUnlocked(items);return items[index];});}
export async function getProviderWorkSummary(){const items=await getProviderWorkQueue();return {total:items.length,planned:items.filter(x=>x.status==="PLANNED").length,blocked:items.filter(x=>x.status==="BLOCKED").length,completed:items.filter(x=>x.status==="COMPLETED").length,errors:items.filter(x=>x.status==="ERROR").length,byProvider:items.reduce<Record<string,number>>((a,x)=>(a[x.primary]=(a[x.primary]??0)+1,a),{})};}
