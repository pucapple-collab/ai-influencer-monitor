...
export async function GET() {
  const [characters, jobs, executions, publishAudit, sizes] = await Promise.all([
    readCharacters(),
    readContentJobs(),
    getExecutions(),
    getPublishAudit(),
  ...
  return NextResponse.json({
    ...
    counts: { characters: characters.length, jobs: jobs.length, executions: executions.length, publishAudit: publishAudit.length },
