...
RUNTIME_HEALTH="$(curl -fsS http://127.0.0.1:3000/api/runtime-health)"
WIDGET_STATUS="$(curl -fsS http://127.0.0.1:3000/api/widget-status)"
if ! printf '%s' "$RUNTIME_HEALTH" | grep -q '"ok":true'; then
  echo "Runtime health check failed"
  exit 1
fi
if ! printf '%s' "$WIDGET_STATUS" | grep -q '"ok":true'; then
  echo "Widget status check failed"
  exit 1
fi
mkdir -p runtime
